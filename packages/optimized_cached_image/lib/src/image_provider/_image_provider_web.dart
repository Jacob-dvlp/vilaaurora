...
class OptimizedCacheImageProvider
    ...
    assert(() {
      collector = () {
        return <DiagnosticsNode>[
          ...
          DiagnosticsProperty<OptimizedCacheImageProvider>('Image key', key),
        ];
      };
      return true;
    }());
  ...
  Stream<ui.Codec> _loadAsyncHttpGet(
    ...
    assert(key == this);
    try {
      final BaseCacheManager mngr = cacheManager ?? DefaultCacheManager();
      await for (FileResponse result in mngr.getFileStream(key.url,
  ...
  bool operator ==(Object other) {
    ...
    if (other is OptimizedCacheImageProvider) {
      final bool sameKey = (cacheKey ?? url) == (other.cacheKey ?? other.url);
